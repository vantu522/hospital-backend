// Middleware validate khung giờ khám
export function validateScheduleSlot(req, res, next) {
  const errors = [];
  const { doctor, time, capacity } = req.body;

  // Kiểm tra doctor
  if (!doctor) {
    errors.push('Trường doctor là bắt buộc');
  }
